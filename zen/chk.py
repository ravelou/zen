...
def getNextForgeRound(seed, **kw):
    try:
        forging_queue = requests.get(
            seed+"/api/delegates/getNextForgers?limit=%(delegates)d" % kw, timeout=0.1).json().get("delegates", [])
    except:
        forging_queue = []
    try:
        delay_before_forge = float2minsec(forging_queue.index(
            kw["publicKey"]) * kw["blocktime"] / 60.)
...
def rebuild():
    ...
    snapshots = [os.stat(os.path.join(snapshot_folder, f))
    ...
    try:
        ...
        new_snapshot_size = os.stat(
            "%(homedir)s/snapshots/%(database)s" % config).st_size
...
def check():
    ...
    # 60/blocktime => blockchain speed in block per minute
    if block_speed < 0.8*60.0/config["blocktime"]:
        status["block speed issue round"] = status.get(
            "block speed issue round", 0)+1
        logMsg("Block speed issue : %.2f blk/min instead of %.2f (round %d)" %
               (block_speed, 60.0/config["blocktime"],
                status["block speed issue round"]))
    else:
        status.pop("block speed issue round", False)

    # if node height is far from half day produced block
    if height_diff > 60./config["blocktime"]*60*12:
        logMsg("Node too far away from height, better rebuild ! (%d blocks)" %
               (height_diff))
        rebuild()
        restart()
    # node height is same as net height
        ...
        if height_diff > config["blocktime"]/0.8:
            logMsg("Height difference is too high ! (%d blocks)" %
                   (height_diff))
        ...
        # node is going solo --> fork !
            ...
            logMsg(
                "Node is going solo with %d blocks forward ! It is forking..." % (-height_diff))
...
def dbbackup():
    """backup the blackchain database config['database'] in the snapshot folder
    config[homedir]/snapshots/
    
    """
    status = loadStatus()
    config = loadConfig()
    if status.get('sync', False) and not status.get("rebuilding", False):
        try:
            execute("pg_dump -Fc %(database)s > %(homedir)s/snapshots/%(database)s",
                    database=config["database"],
                    homedir=config["homedir"]
                   )
        except Exception:
