...
def setup():
    ...
    config["seeds"] = ["http://%(ip)s:%(port)s" %
                       item for item in _cnf.get("peers", {}).get("list", [])]
    ...
    account = requests.get(seed+"/api/delegates/get?publicKey=%s" %
                           keys["publicKey"], verify=True, timeout=5).json().get("delegate", {})
    tbw_config["username"] = account["username"]
    tbw_config["excludes"] = [account["address"]]
    account = requests.get(seed+"/api/accounts?address=%s" %
                           account["address"], verify=True, timeout=5).json().get("account", {})

    if account.get("secondPublicKey", False):
        seed = tbw_config.get("#2", "01")
        secondPublicKey = crypto.getKeys(
            None, seed=crypto.unhexlify(seed))["publicKey"]
        while secondPublicKey != account["secondPublicKey"]:
            try:
                secret = getpass.getpass("> enter your second secret: ")
                seed = crypto.hashlib.sha256(secret.encode(
                    "utf8") if not isinstance(secret, bytes) else secret).digest()
...
def configure():
    ...
    if OPTIONS.funds:
        if requests.get(config["peer"]+"/api/accounts?address=%s" % OPTIONS.funds, verify=True, timeout=5).json().get("success", False):
            ...
            sys.stdout.write(
                "> %s seems not to be a valid address" % OPTIONS.funds)
    if OPTIONS.vendorField:
        tbw_config["vendorField"] = OPTIONS.vendorField[0:min(
            64, len(OPTIONS.vendorField))]
    if OPTIONS.excludes:
...
def getBestSeed(*seeds):
    ...
    for seed in seeds:
        try:
            h = requests.get(seed+"/api/blocks/getHeight",
                             verify=True, timeout=5).json().get("height", 0)
            if h > height:
                height = h
                url = seed
...
def execute(*lines, **config):
    ...
    for line in [l % config for l in lines]:
        ...
        sys.stdout.flush()
        err += "... %s\n" % line
        answer = subprocess.Popen(shlex.split(
...
def bash(*lines, **config):
    answer = subprocess.Popen(
        "/bin/bash", stdout=subprocess.PIPE, stderr=subprocess.PIPE)
